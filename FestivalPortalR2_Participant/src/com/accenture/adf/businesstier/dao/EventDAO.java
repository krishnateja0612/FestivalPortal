...
public class EventDAO {
	...
	public ArrayList<Object[]> showAllEvents(String eventname)
		...
		resultSet = statement.executeQuery();
		log.info("All Events retreived in descending order from Database :" + eventList);
		while (resultSet.next()) {
			Object[] eventObject = new Object[8];
			eventObject[0] = resultSet.getInt("eventid");
			eventObject[1] = resultSet.getString("name");
	...
	public Event getEvent(int eventId, int sessionId)
		...
		log.info("All events retrieved from the database for eventId: "+eventId);
		return event;
	}
	...
	public int updateEvent(Event updateEvent) throws ClassNotFoundException,
		...
		log.info("Event Updated in the database Event with eventid: "+updateEvent.getEventid());
	    FERSDataConnection.closeConnection();
		connection=FERSDataConnection.createConnection();
		statement=connection.prepareStatement(query.getUpdateEventSession());
		statement.setInt(1, Integer.parseInt(updateEvent.getSeatsavailable()));
		statement.setInt(2, updateEvent.getSessionId());
		statement.setInt(3, updateEvent.getEventid());
		updateStatus=statement.executeUpdate();
		log.info("EventSession details updated for sessionId: "+updateEvent.getEventSession()+" and eventId: "+updateEvent.getEventid());
		return updateStatus;
	}
	...
	public int deleteEvent(int eventId, int sessionId)
		...
		status=statement.executeUpdate();
		 FERSDataConnection.closeConnection();
			...
			statement.setInt(1,eventId);
			status=statement.executeUpdate();
			log.info("EventSession details deleted");
		return status;	
	}
	...
	public List<EventCoordinator> getEventCoordinator()
		...
		resultSet = statement.executeQuery();
		log.info("fetching eventcoordinator details in descending order from Database " );
		while(resultSet.next()){
